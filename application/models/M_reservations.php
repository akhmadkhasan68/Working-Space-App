<?php
    class M_reservations extends CI_Model{
        public function get_reservations($user_id = null, $status_reservation = null, $status_payment = null, $place_id = null)
        {
            $this->db->select('a.*, b.type, h.name, b.address, b.phone, b.created_at pay_date, c.total_payment, c.photo, c.status status_payment, d.value rating, e.description feedback, f.id payment_id, f.name payment, g.name place, g.price');
            ...
            $this->db->join('ratings d', 'd.reservation_id = c.id', 'left');
            ...
            $this->db->join('users h', 'h.id = a.user_id', 'left');
            if($user_id != null) $this->db->where('a.user_id', $user_id);
            if($place_id != null) $this->db->where('g.id', $place_id);
        ...
        {
            $this->db->select('a.*, b.type, b.name as name_detail, h.name, b.address, b.phone, b.type instansi, b.created_at pay_date, c.total_payment, c.photo, c.status status_payment, d.value rating, e.description feedback, f.id payment_id, f.name payment');
            $this->db->from('reservations a');
            $this->db->join('reservation_detail b', 'b.reservation_id = a.id', 'left');
            $this->db->join('transaction c', 'c.reservation_id = a.id', 'left');
